...
class AddUsageTrackingToRagdollEmbeddings < ActiveRecord::Migration[8.0]
  def change
    add_column :ragdoll_embeddings, :usage_count, :integer, default: 0, null: false unless column_exists?(:ragdoll_embeddings, :usage_count)
    add_column :ragdoll_embeddings, :returned_at, :timestamp unless column_exists?(:ragdoll_embeddings, :returned_at)
    
    # Add indexes for performance when sorting by usage metrics
    add_index :ragdoll_embeddings, :usage_count, name: 'index_ragdoll_embeddings_on_usage_count' unless index_exists?(:ragdoll_embeddings, :usage_count)
    add_index :ragdoll_embeddings, :returned_at, name: 'index_ragdoll_embeddings_on_returned_at' unless index_exists?(:ragdoll_embeddings, :returned_at)
