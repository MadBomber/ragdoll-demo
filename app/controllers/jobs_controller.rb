class JobsController < ApplicationController
  ...
  def bulk_delete
    ...
    redirect_to jobs_path, notice: "Successfully deleted #{deleted_count} job(s)."
  rescue => e
    redirect_to jobs_path, alert: "Bulk delete failed: #{e.message}"
  end
  ...
  def bulk_retry
    ...
    redirect_to jobs_path, notice: "Successfully retried #{retried_count} job(s)."
  rescue => e
    redirect_to jobs_path, alert: "Bulk retry failed: #{e.message}"
  end

  def cancel_all_pending
    begin
      deleted_count = SolidQueue::Job.where(finished_at: nil).delete_all
      redirect_to jobs_path, notice: "Successfully canceled #{deleted_count} pending job(s)."
    rescue => e
      redirect_to jobs_path, alert: "Failed to cancel all pending jobs: #{e.message}"
    end
  end
end